  ...
  return (
    <HoverCard openDelay={300} closeDelay={150}>
      <HoverCardTrigger asChild>
        <div className="group relative rounded overflow-hidden transition-all duration-300 hover:scale-105 hover:z-10 cursor-pointer">
            ...
            {showWatchedBy.length > 0 && (
              <div className="flex items-center space-x-2 text-sm">
                <div className="flex -space-x-1">
                  {showWatchedBy.slice(0, 3).map((friend, index) => (
                    <div key={index} className="w-6 h-6 rounded-full bg-gradient-to-r from-blue-400 to-purple-500 border-2 border-card flex items-center justify-center text-white text-xs font-bold">
                      {friend[0]}
                    </div>
                  ))}
                  {showWatchedBy.length > 3 && (
                    <div className="w-6 h-6 rounded-full bg-muted border-2 border-card flex items-center justify-center text-xs font-bold text-muted-foreground">
                      +{showWatchedBy.length - 3}
                    </div>
                  )}
                </div>
